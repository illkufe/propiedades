...
class ProcesosRevisionForm(forms.ModelForm):

	def __init__(self, *args, **kwargs):

		super(ProcesosRevisionForm, self).__init__(*args, **kwargs)
		self.fields['tipo_estado'].queryset 				= Tipo_Estado_Proceso.objects.filter(id=2)
		self.fields['tipo_estado'].widget.attrs['readonly'] = True
	...
	class Meta:
		...
		widgets = {
			'id'				: forms.NumberInput(attrs={'class': 'form-control', 'type': 'hidden'}),
			'tipo_estado'		: forms.Select(attrs={'class': 'form-control'}),
			'nombre'		    : forms.TextInput(attrs={'class': 'form-control select-antecesor'}),
			'responsable'		: forms.SelectMultiple(attrs={'class': 'select2 form-control', 'multiple': 'multiple'}),
			'antecesor'			: forms.SelectMultiple(attrs={'class': 'select2 form-control', 'multiple': 'multiple'}),
...
class ProcesosAprobacionForm(forms.ModelForm):

	def __init__(self, *args, **kwargs):

		super(ProcesosAprobacionForm, self).__init__(*args, **kwargs)
		self.fields['tipo_estado'].queryset 				= Tipo_Estado_Proceso.objects.filter(id=3)
		self.fields['tipo_estado'].widget.attrs['readonly'] = True
